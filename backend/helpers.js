...
export const log = (level, module, message, data = null) => {
	...
	console[level](
		`[${level.toUpperCase()}] [${timeStamp}] [${module}] - [${message}]`,
		data || "",
	);
};
...
export const checkAuthHelper = (req, res, next) => {
	if (!req.session.user) {
		return res
			.status(401)
			.json({ success: false, user: null, message: "User not authenticated" });
	}
...
export const fetchYahooBuySell = async (symbol, route, userId) => {
	try {
		...
		log("info", route, "Fetched stock price successfully: fetchYahooBuySell", {
			symbol: price,
		});
		return price;
	} catch (err) {
