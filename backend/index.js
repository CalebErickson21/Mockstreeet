...
app.get("/api/check-auth", (req, res) => {
	try {
		if (req.session.user) {
			...
			return res
				.status(200)
				.json({ success: false, user: null, message: "User is not authenticated" }); // Still successful because users can be on homepage and not be authenticated
		}
	} catch (err) {
		log("error", "/check-auth", `Authentication error: ${err.message}`, {
			user: getSessionUserId(req),
		});
...
app.post("/api/login", async (req, res) => {
	try {
		...
		const queryRes = await db.query("SELECT * FROM users WHERE username = $1 OR email = $1;", [
			userNameOrEmail,
		]);
...
app.post("/api/register", async (req, res) => {
	try {
		...
		if (email.length > 50) {
			return res
				.status(400)
				.json({ success: false, message: "Email must be less than 50 characters" }); // User error (400)
		}
		...
		if (existsResult.rows.length > 0) {
			return res
				.status(400)
				.json({ success: false, message: "Username or email already exists" }); // User error (400)
		}
		...
		try {
			...
			await db.query("INSERT INTO portfolios (user_id, portfolio_name) VALUES ($1, $2);", [
				registerRes.rows[0].user_id,
				"All",
			]);
...
app.get("/api/portfolio/names", checkAuthHelper, async (req, res) => {
	try {
		...
		if (portfolioQuery.length === 0) {
			log("error", "/portfolio/names", "User does not have any portfolios", {
				user: req.session.user.user_id,
			});
			return res
				.status(404)
				.json({ success: false, portfolioNames: [], message: "Internal server error." });
		}
...
app.get("/api/portfolio/stocks", checkAuthHelper, async (req, res) => {
	try {
		...
		if (portfolioName !== "All") {
			// Default portfolio is container for all stocks not in portfolio, but want to show all stocks on frontend
		...
		if (portfolioIds.length === 0) {
			log("error", "/portfolio/stocks", "Portfolio not found", {
				user: req.session.user.user_id,
			});
		...
		try {
			...
			const stockPrices = await fetchStockSearch(
				stockSymbols,
				"/portfolio/stocks",
				req.session.user.user_id,
			);
			...
			return res
				.status(200)
				.json({ success: true, stocks: stockData, value: portfolioValue.toFixed(2) });
		} catch (err) {
			// Error handling
			log("error", "/portfolio/stocks", `Fetch stocks error: ${err.message}`, {
				user: req.session.user.user_id,
			});
			return res.status(500).json({ success: false, message: "Internal server error." });
		}
	} catch (err) {
		log("error", "/portfolio/stocks", `Error returning portfolio stocks: ${err.message}`, {
			user: req.session.user.user_id,
		});
		return res.status(500).json({ success: false, message: "Internal server error" });
	}
...
app.post("/api/portfolio/new", checkAuthHelper, async (req, res) => {
	try {
		...
		if (portfolioName === "All") {
			...
			return res
				.status(400)
				.json({ success: false, message: '"All" is an invalid portfolio name' });
		}
		if (portfolioName === "Createnew" || portfolioName === "Create New") {
			// Invalid portfolio name
			return res
				.status(400)
				.json({ success: false, message: '"Create New" is an invalid portfolio name' });
		}
		...
		try {
			...
			if (existsResult.length !== 0) {
				return res
					.status(400)
					.json({ success: false, message: "Portfolio name already exists " });
			}

			// Insert portfolio into database
			await db.query("INSERT INTO portfolios (portfolio_name, user_id) VALUES ($1, $2);", [
				portfolioName,
				req.session.user.user_id,
			]);
			...
			log("error", "/portfolio/new", `Error creating portfolio: ${err.message}`, {
				user: req.session.user.user_id,
			});
			return res.status(500).json({ success: false, message: "Internal server error" });
		}
...
app.get("/api/transactions", checkAuthHelper, async (req, res) => {
	try {
		...
		if (!portfolioFilter || !startFilter || !endFilter || !transactionFilter || !stockFilter) {
			log("error", "/portfolio/transactions", "Empty input fields", {
				user: req.session.user.user_id,
			});
			return res
				.status(400)
				.json({ success: false, message: "All input fields required", transactions: [] });
		}
		// Error check inputs
		...
		if (
			...
			log("error", "/portfolio/transactions", "Invalid transaction filter", {
				user: req.session.user.user_id,
			});
			return res
				.status(400)
				.json({ success: false, message: "Invalid transaction filter", transactions: [] });
		}

		// Get query and params
		let transactionQuery =
			"SELECT symbol, transaction_type, shares, share_price, (shares * share_price) AS total_price, transaction_date FROM transactions t JOIN portfolios p ON t.portfolio_id = p.portfolio_id WHERE p.user_id = $1 AND t.transaction_date BETWEEN $2 AND $3";
		...
		if (queryRes.length === 0) {
			...
			return res
				.status(200)
				.json({ success: true, message: "No transactions", transactions: [] });
		}
		...
		log("error", "/portfolio/transactions", `Error: ${err.message}`, {
			user: req.session.user.user_id,
		});
...
app.get("/api/watchlist", checkAuthHelper, async (req, res) => {
	try {
		...
		// Fetch stock data
		const stockData = await fetchStockSearch(
			stockSymbols,
			"/watchlist",
			req.session.user.user_id,
		);
		if (!stockData) {
			return res
				.status(500)
				.json({ success: false, watchlist: [], message: "Internal server error." });
		}
		...
		log("error", "/portfolio/watchlist", `Error: ${err.message}`, {
			user: req.session.user.user_id,
		});
		return res.status(500).json({ success: false, message: "Internal server error." });
	}
...
app.post("/api/watchlist/add", checkAuthHelper, async (req, res) => {
	try {
		...
		if (!portfolioName || !stockSymbol) {
			log("error", "/portfolio/watchlist/add", "Empty input fields", {
				user: req.session.user.user_id,
			});
		...
		if (portfolioName.length > 50 || stockSymbol.length > 10) {
			log("error", "/portfolio/watchlist/add", "Invalid input fields", {
				user: req.session.user.user_id,
			});
		...
		try {
			...
			if (portfolioIdRes.length !== 1) {
				log("error", "/portfolio/watchlist/add", "Portfolio not found", {
					user: req.session.user.user_id,
				});
				return res.status(404).json({ success: false, message: "Portfolio not found" });
			}
			...
			await db.query("INSERT INTO watchlists (portfolio_id, symbol) VALUES ($1, $2);", [
				portfolioId,
				stockSymbol,
			]);
			...
			// Rollback transaction if error
			await db.query("ROLLBACK;");
			log("error", "/portfolio/watchlist/add", `Error adding stock: ${err.message}`, {
				user: req.session.user.user_id,
			});
			return res.status(500).json({ success: false, message: "Internal server error" });
		}
		...
		log("error", "/portfolio/watchlist/add", `Error: ${err.message}`, {
			user: req.session.user.user_id,
		});
		return res.status(500).json({ success: false, message: "Internal server error" });
	}
...
app.post("/api/watchlist/remove", checkAuthHelper, async (req, res) => {
	try {
		...
		// Error check inputs
		if (!portfolioName || !stockSymbol) {
			log("error", "/portfolio/watchlist/rempve", "Empty input fields", {
				user: req.session.user.user_id,
			});
			return res.status(400).json({ success: false, message: "All input fields required" });
		}
		if (portfolioName.length > 50 || stockSymbol.length > 10) {
			log("error", "/portfolio/watchlist/remove", "Invalid input fields", {
				user: req.session.user.user_id,
			});
			return res.status(400).json({ success: false, message: "Invalid input fields" });
		}
		...
		try {
			...
			if (portfolioIdRes.length !== 1) {
				log("error", "/portfolio/watchlist/remove", "Portfolio not found", {
					user: req.session.user.user_id,
				});
				return res.status(404).json({ success: false, message: "Portfolio not found" });
			}
			...
			await db.query("DELETE FROM watchlists WHERE portfolio_id = $1 AND symbol = $2;", [
				portfolioId,
				stockSymbol,
			]);
			...
			// Rollback transaction if error
			await db.query("ROLLBACK;");
			log("error", "/portfolio/watchlist/remove", `Error removing stock: ${err.message}`, {
				user: req.session.user.user_id,
			});
			return res.status(500).json({ success: false, message: "Internal server error" });
		}
		...
		log("error", "/portfolio/watchlist/remove", `Error: ${err.message}`, {
			user: req.session.user.user_id,
		});
		return res.status(500).json({ success: false, message: "Internal server error" });
	}
...
app.get("/api/market/search", checkAuthHelper, async (req, res) => {
	try {
		const searchStocksList = req.query.stock
			?.split(",")
			.map((s) => formatStockTransaction(s))
			.filter((s) => s !== null);

		// Error check inputs
		if (!searchStocksList) {
			log("error", "/market/search", "Invalid search input", {
				user: req.session.user.user_id,
			});
...
app.post("/api/market/buy", checkAuthHelper, async (req, res) => {
	try {
		...
		try {
			...
			// Fetch stock price
			const stockPrice = await fetchYahooBuySell(
				stockSymbol,
				"/market/buy",
				req.session.user.user_id,
			);
			...
			if (totalCost > balance) {
				log("error", "/market/buy", "Insufficient funds", {
					user: req.session.user.user_id,
				});
			...
			if (portfolioIdRes.length !== 1) {
				log("error", "/market/buy", "Portfolio not found", {
					user: req.session.user.user_id,
				});
				return res.status(404).json({ success: false, message: "Portfolio not found" });
			}
			...
			// Update user balance
			await db.query("UPDATE users SET balance = balance - $1 WHERE user_id = $2;", [
				totalCost,
				req.session.user.user_id,
			]);
			...
			// Rollback transaction if error
			await db.query("ROLLBACK;");
			log("error", "/market/buy", `Error buying stock: ${err.message}`, {
				user: req.session.user.user_id,
			});
			return res.status(500).json({ success: false, message: "Internal server error" });
		}
...
app.post("/api/market/sell", checkAuthHelper, async (req, res) => {
	try {
		...
		if (isNaN(numShares)) {
			log("error", "/market/sell", "Invalid shares input", {
				user: req.session.user.user_id,
			});
			return res.status(400).json({ success: false, message: "Shares must be a number" });
		}
		if (portfolioName.length >= 50 || stockSymbol.length >= 10 || shares <= 0) {
			log("error", "/market/sell", "Invalid input fields", {
				user: req.session.user.user_id,
			});
			return res.status(400).json({ success: false, message: "Invalid input fields" });
		}
		...
		try {
			...
			// Rollback transaction if error
			await db.query("ROLLBACK;");
			log("error", "/market/sell", `Error buying stock: ${err.message}`, {
				user: req.session.user.user_id,
			});
			return res.status(500).json({ success: false, message: "Internal server error" });
		}
...
app.get("/api/balance", checkAuthHelper, async (req, res) => {
	try {
		...
		if (queryRes.length !== 1) {
			log("error", "/user/stats", "More than one or no user", {
				user: req.session.user.user_id,
			});
			return res.status(500).json({ success: false, message: "Internal server error" });
		}
		...
		return res
			.status(200)
			.json({ success: true, balance: balance, message: "Balance fetched successfully" });
	} catch (err) {
...
app.post("/api/balance/add", checkAuthHelper, async (req, res) => {
	try {
		...
		if (increment <= 0) {
			log("info", "/balance/add", "Input must be positive", {
				user: req.session.user.user_id,
			});
		...
		try {
			...
			if (queryRes[0].balance + increment >= MAX_NUM) {
				...
				return res
					.status(400)
					.json({ success: false, message: "Invalid increment input." });
			}
			...
			// Rollback if error
			await db.query("ROLLBACK;");
			log("error", "/balance/add", `Database error ${err.message}`, {
				user: req.session.user.user_id,
			});
			return res.status(500).json({ success: false, message: "Internal server error" });
		}
